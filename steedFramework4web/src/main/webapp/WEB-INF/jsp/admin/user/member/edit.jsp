...
<form target="navTab" method="post" action="<st:getCurrentUrl lastUrl="update.act" />" class="pageForm required-validate" onsubmit="return addMember(this);">
	<fieldset>
		<legend>基本信息</legend>
		<dl>
			<dt>ID:</dt>
		...
		<dl class="nowrap">
			...
			<dd>
				<c:forEach items="${roleNameList}" var="temp" varStatus="status">
					<input type="checkbox" 
					<c:if test="${domain.roleSet.contains(temp) }"> checked="checked"</c:if> name="roleSet" value="${temp }"/>
